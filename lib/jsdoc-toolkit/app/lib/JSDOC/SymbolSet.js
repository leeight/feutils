...
JSDOC.SymbolSet.prototype.walk = function(symbol) {
    ...
    var type = augments[i].type;
		var contributer = this.getSymbol(type);
		if (!contributer && JSDOC.Lang.isBuiltin(''+type)) {
			contributer = new JSDOC.Symbol("_global_."+type, [], type, new JSDOC.DocComment("Built in."));
		...
		else LOG.warn("Can't augment contributer: "+type+", not found.");
	}
}
