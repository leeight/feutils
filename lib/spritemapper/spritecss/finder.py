...
icon_url_re = re.compile(r'.*icon(-|_)[^\\\/]+$')
...
_pos_units = ("px", "%")
_repeat_names = ("repeat", "repeat-x", "repeat-y")
...
def _bg_repeated(val):
    parts = val.split()
    # songao(TODO): what about default background repeat(->repeat)?
    for p in parts:
        if p in _repeat_names:
            return True
    return False
...
def _match_background_url(val):
    ...
    full_url = mo.groups()[0]
    # songao: only mapping the images whose name starts with "icon-" or "icon_"
    if icon_url_re.match(full_url):
        return full_url
    else:
        raise NoSpriteFound(val)

def get_background_url(val):
    # songao: check if the image's name starts with "icon(-|_)" first.
    img_url = _match_background_url(val)
    is_positioned = _bg_positioned(val)
    is_repeated = _bg_repeated(val)
    if img_url and not is_positioned and not is_repeated:
        return img_url
    elif img_url:
        logger.error("malformed background : \"%s\", there should not be any position or repeat property in background", val)
        sys.exit(1)
    else:
