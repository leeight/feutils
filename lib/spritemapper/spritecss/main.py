...
def get_signed_path(resource):
    sign = hashlib.md5(open(resource, 'rb').read()).hexdigest()
    dn, bn = path.split(resource)
    base, ext = path.splitext(bn)
    name = "%s-%s%s" % (base, sign, ext)
    return path.join(dn, name)


def spritemap(css_fs, conf=None, out=sys.stderr):
    w_ln = lambda t: logging.info(t)
    ...
    for smap in smaps:
        with open_sprites(smap, pad=conf.padding) as sprites:
            ...
            # sign the file
            if conf.sign_output:
                signed_path = get_signed_path(smap.fname)
                rename(smap.fname, signed_path)
                w_ln("Rename spritemap image to %s" % (signed_path,))
                smap.sname = signed_path
...
op.add_option("--sign_output", default=False, action="store_true",
              help="to sign the output sprites or not")
...
def main():
    ...
    base["sign_output"] = opts.sign_output
