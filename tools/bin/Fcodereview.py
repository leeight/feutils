...
DEFAULT_REVIEW_SERVER = "fe.baidu.com/codereview"
...
class HttpRpcServer(AbstractRpcServer):
  ...
  def _GetCRSFToken(self, login_page):
    req = self._CreateRequest(url = login_page)
    res = self.opener.open(req)
    txt = res.read()
    # name='csrfmiddlewaretoken' value='6d715f01c35dccd0ef664ff3a11b18fb'
    start = txt.find("name='csrfmiddlewaretoken' value='")
    end = txt.find("'", start + 34)
    return txt[start+34:end]
  ...
  def _Authenticate(self, login_url="/accounts/login/"):
    ...
    csrf_token = self._GetCRSFToken(login_url)
    req = self._CreateRequest(
        url = login_url,
        data = urllib.urlencode({
            "username": username,
...
def RunShellWithReturnCode(command, print_output=False,
  ...
  Returns:
    Tuple (output, return code, errout)
  """
    ...
    if status[0] == "A" and status[3] != "+":
      ...
      if get_base:
        ...
        if self.rev_start:
          ...
          url = "%s/%s@%s" % (self.svn_base, filename, self.rev_start)
          base_content, ret_code, errout = RunShellWithReturnCode(["svn", "cat", url],
...
class PerforceVCS(VersionControlSystem):
  ...
  def RunPerforceCommandWithReturnCode(self, extra_args, marshal_output=False,
    ...
    data, retcode, errout = RunShellWithReturnCode(
