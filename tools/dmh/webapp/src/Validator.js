...
var Validator = (function() {
            ...
            'link' : {
                validate: function(text) {
                    ...
                    if (len === 0) {
                        ...
                        return 2;
                    }else if (!dn.util.regexp.urlLoose.test(text)) {
                        return 3;
                    }
                },
                notice: noticeInTail,
    ...
    function cancelNoticeInTile(input) {
        ...
        for (var i = 0; i < father.childNodes.length; i++) {
            ...
            if (child.className === validClass && child.style.display !== 'none') {
                noError = false;
                break;
            }
        }
        ...
        if (segLen > 0) {
            for (i = 1; i < segLen; i++) {
                if (ruleSeg[i] == 'this') {
                    ...
                    ctrl = ui.util.get(ruleSeg[i], control.getPage());
 ...
 * @param {number=} opt_width 若需显示的控件宽度超过父层宽度，可设置错误层显示的宽度，若使用请考虑对parentNode的宽度影响.
 */
