...
ui.SlotSelector.prototype = (function() {
    ...
    function getMultiSitesBudget() {
        ...
        baidu.array.each(tbl.datasource, function(item, i) {
    ...
    function onSearchMultiSites(tbl, cBox, txtInput, community_id) {
        ...
        this.slotlistRequester(param, function(data) {
            tbl.model.datasource = data.page.result;
            tbl.rebindModel();
    ...
    function initCheckDisplay_tblMultiSitesLeft() {
        var me = this,
            main_chkAllId = me.c('tblMultiSitesLeft').getId('selectAll'),
            el_main_chkAllId = baidu.g(main_chkAllId),
    ...
    function initCheckDisplay_tblMultiSitesLeft_SingleSublist(subrowTable, communityId) {
        var el_subrow_item_chkId, me = this,
            //第一次初始化subrowTable.datasource为空
            data = subrowTable.datasource;
    ...
    function getSelected_tblMultiSitesLeft(ctrl) {
        ...
        baidu.array.each(me.datasourceMultiSitesLeft, function(item, index) {
            ...
            if (baidu.g(mainrow_item_chkId).checked) {
                if (me.communityOriginSlot[item.id]) {
                    selected.mainList.push(item);
                    if (subrowTable) {
                        selected.subLists.push(subrowTable.datasource.slice(0));
                    } else {
                        if (me.communityOriginSlot[item.id]) {
                            selected.subLists.push(me.communityOriginSlot[item.id]);
                        }
                    }
                }
            }
    ...
    function refreshMultiSites_Right(ctrl) {
        ...
        if (ctrl.cacheCommunityId) {
            ...
            baidu.array.each(tblTarget.datasource, function(item, i) {
                if (item.id == ctrl.cacheCommunityId) {
                    tblTarget.fireSubrow(i);
                    return false;
                }
            });
    ...
    function refreshMultiSites_Left(ctrl) {
        ...
        baidu.array.each(mainTable.datasource, function(communityItem, index) {
            subrowTable = me.c('tblMultiSitesLeft').c(SUBTABLE_IDPREFIX_LEFT + index);
            if (subrowTable) {
                var data = subrowTable.datasource;
    ...
    function refreshSingleSite(ctrl) {
        var leftList = ctrl.c('tblSingleSiteLeft'),
            ...
            data = leftList.datasource, dataItem,
            dataLen = data.length,
     ...
     * 展开结果页面subrow
     */
    function onShowResultSubList(index, dataItem) {
        var me = this;
        var mainTable = me.c('tblMultiSitesResult'),
        controlId = SUBTABLE_IDPREFIX_RESULT + index,
        subDatasource = me.viewValue['multiSites'].subLists[index].slice(0),
        table;
        ...
        if (!me.c('tblMultiSitesResult').c(controlId)) {
            ...
            var listContainer = baidu.g(mainTable.getId('subListTable' + index));
    ...
    function chkAllStatus_tblSingleSiteLeft(ctrl) {
        var leftList = ctrl.c('tblSingleSiteLeft'),
            ...
            chkidPrefix = leftList.getId('multiSelect'),
            hasAllSelected = true;
        baidu.array.each(leftList.datasource, function(item, i) {
    ...
    function getSelectedSlotlistByCommunityId(communityId) {
        var rtn = [], me = this;
        baidu.array.each(me.viewValue['multiSites'].mainList, function(item, i) {
    ...
    function rebindSublist_MultiSitesTbl() {
        ...
        this.communityslotRequester(param, function(data) {
            ...
            me.communityList = communityList;
            me.communityOriginSlot = {};
            for (i = 0; i < dataCommunityLen; i++) {
                  slotlist = baidu.array.find(communityList, function(item, index) {
                      return item.id == dataCommunity[i].id;
                  });

                  //此对象中只保存slotlist信息，供subrow创建及批量广告位添加时使用
                  me.communityOriginSlot[dataCommunity[i].id] = slotlist ? slotlist.slotlist.slice(0) : [];
                  baidu.g(mainTable.getId('multiSelect' + i)).checked = false;

                  //rebind 列表
                  subrowTable = mainTable.c(SUBTABLE_IDPREFIX_LEFT + i);
                  if (subrowTable) {
                    subrowTable.datasource = slotlist ? slotlist.slotlist.slice(0) : [];
                    ...
                    comboBox.model.datasource = me.datasourceSlotStatus;
    ...
    function rebind_SingleSiteTbl() {
        ...
        me.slotlistRequester(param, function(data) {
            mainTable.model.datasource = data.page.result;
            mainTable.rebindModel();
            
            me.c('listPagerSingleSiteLeft').rebindModel(pagerHelper.call(me, data.page.totalCount));
        });
        me.c('comboSearchStatus').model.datasource = me.datasourceSlotStatus;
     ...
     * 创建左侧subrow
     */
    function onShowSingleSubListLeft(communityId, index) {
        ...
        if (!this.c('tblMultiSitesLeft').c(controlId)) {
            //从communityList中筛选出当前生活圈对应的广告位数据
            data = this.communityOriginSlot[communityId] ? this.communityOriginSlot[communityId] : [];
            ...
            if (!me.c('tblMultiSitesLeft').c(controlId)) {
                ...
                //绑定maintable选中时subrow的自动全选
                var subrow_title_chkId = table.getId() + 'selectAll';
                baidu.hide(subrow_title_chkId);
                mainrow_item_chkId = mainTable.getId('multiSelect' + index);
                ...
                //如果创建前maintable已选中，则创建时自动全选
                if (baidu.g(mainrow_item_chkId).checked) {
                  baidu.g(subrow_title_chkId).click();
                }

                //子列表item选框点击事件
                  initCheckDisplay_tblMultiSitesLeft_SingleSublist.call(me, table, communityId);
     ...
     * 创建右侧subrow
     */
    function onShowSingleSubListRight(communityId, index) {
        var controlId = SUBTABLE_IDPREFIX_RIGHT + index,
            me = this,
            data,
            mainTable = this.c('tblMultiSitesRight');
        var tpl = '<div id="{0}" class="cb-sublist-loading">{2}</div>' +
            '<div id="{1}" class="cb-sublist-table" style="display:none"></div>';

        if (!this || !this.c || !this.c('tblMultiSitesRight')) return;
        if (!this.c('tblMultiSitesRight').c(controlId)) {
            //从communityList中筛选出当前生活圈对应的广告位数据
            data = this.viewValue['multiSites'].subLists[index];
            //data = subListData ? subListData.slotlist : [];
     ...
     * 展开执行前的准备
     */
    function onToggleSubList(targetDirection, type, index, dataItem) {
        //show source sublist
        var subListData, targetTable;
        var me = this;
        if (type == 'open') {
            //如果subrow控件还未创建，则先创建
            if (targetDirection == 'left') {
                onShowSingleSubListRight.call(me, dataItem.id, index);
            }
            else {
                onShowSingleSubListLeft.call(me, dataItem.id, index);
            }
        }

        // syncCollapse target sublist
        syncCollapse.call(me, targetDirection, type, index, dataItem);
    }
    ...
    function syncCollapse(targetDirection, type, index, dataItem) {
        ...
        baidu.array.each(tblTarget.datasource, function(item, i) {
            if (item.id == communityId) {
                //需同步展开的控件可能未建立，要先创建
                if (targetDirection == 'left') {
                    onShowSingleSubListLeft.call(me, communityId, i);
                }
                else {
                    onShowSingleSubListRight.call(me, communityId, i);
                }
    ...
    return {
        ...
        renderSubList_tblMultiSitesLeft: function() {
            ...
            this.communityslotRequester(param, function(data) {
                ...
                for (i = 0; i < communityList.length; i++) {
                    if (communityList[i].slotlist.length > 0) me.AllSlotList = me.AllSlotList.concat(communityList[i].slotlist);

                    //创建以communityId为键值的communityListIndex是为了提高community的搜索速度
                    me.communityListIndex[communityList[i].id] = communityList[i];

                    //用于保存community下的原始广告位数据，当执行全选，但是并未建立subrow时直接从这个对象中读取数据，
                    //该对象的键值是communityId。
                    me.communityOriginSlot[communityList[i].id] = communityList[i] ? communityList[i].slotlist.slice(0) : [];
                }
                if (me.noCommunitySlotlist && me.noCommunitySlotlist.length > 0) {
                    me.AllSlotList = me.AllSlotList.concat(me.noCommunitySlotlist);
                }
        ...
        bindEvent: function() {
            ...
            //同步生活圈左右收缩状态
            this.c('tblMultiSitesRight').onsubrowopen = baidu.fn.bind(onToggleSubList, this, 'left', 'open');
            this.c('tblMultiSitesRight').onsubrowclose = baidu.fn.bind(onToggleSubList, this, 'left', 'close');
            this.c('tblMultiSitesLeft').onsubrowopen = baidu.fn.bind(onToggleSubList, this, 'right', 'open');
            this.c('tblMultiSitesLeft').onsubrowclose = baidu.fn.bind(onToggleSubList, this, 'right', 'close');
            this.c('tblMultiSitesResult').onsubrowopen = baidu.fn.bind(onShowResultSubList, this);
...
//                    });
                    var community = me.communityListIndex[communityId];
                    ...
                    if (community) {
                        ...
                        var arr = [];
                        baidu.array.each(/** @type {Array} */(arrId), function(id, i) {
                            var slot = baidu.array.find(me.AllSlotList, function(item, j) {
                                return item.id == id;
                            });
                            if (slot) {
                                arr.push(slot);
                ...
                baidu.array.each(me.selectedSlot.budget_role_ids, function(slotId, index) {
                    ...
                    var slot = baidu.array.find(me.AllSlotList, function(item, i) {
                        return item.id == slotId;
                    });
